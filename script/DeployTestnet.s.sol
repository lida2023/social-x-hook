...
contract DeployTestnet is Script {
    ...
    function run() external {
        ...
        console2.log("=== Deploy SocialX testnet stack ===");
        console2.log("Deployer:", deployer);
        console2.log("Chain:", block.chainid);

        vm.startBroadcast(pk);
        PoolManager poolManager = new PoolManager(deployer);
        TestnetHookDeployer hookDeployer = new TestnetHookDeployer();
        bytes32 salt = _findSalt(address(hookDeployer), address(poolManager), deployer);
        SocialXHook hook = hookDeployer.deploy(salt, address(poolManager), deployer);
        vm.stopBroadcast();
